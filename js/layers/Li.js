...
function LiMineMain(){
    let pow = n(1).mul(10).mul(buyableEffect('li',11)[0]).pow(buyableEffect('li',13)[0])
    return [
        pow,//力量
...
function LiMinePicker(){
    let pow = n(1).mul(buyableEffect('li',21)[1]).mul(buyableEffect('li',13)[1])
    return [
        pow,//力量
        n(20).sub(buyableEffect('li',22)),//冷却
        [//显示
            n(0).mul(pow),
            pow,
        ],
...
addLayer("li", {
    ...
    type: "none",
	effect(){
        let mul = player.li.achievement.pow(12)
	...
	clickables:{
        'mine_main_0':{
            title:'矿洞',
            display(){
                return '挖掘<br>冷却:'+formatTime(player.li.ore_cold[0])+'('+formatTime(LiMineMain()[1])+')<br>力量:'+format(LiMineMain()[2][0],2)+'~'+format(LiMineMain()[2][1],2)
            },
            ...
            onClick(){
                ...
                player.li.totleOre = player.li.totleOre.add(n(Math.random()).mul(LiMineMain()[0])).mul(buyableEffect('li',31))
        ...
        'mine_picker_0':{
            ...
            onClick(){
                player.li.totleOre = player.li.totleOre.add(n(Math.random()).mul(LiMinePicker()[0]))
    ...
    buyables: {
		11: {
			cost(x) { 
				return n(4.5).pow(n(x).add(1)).pow(n(x).div(100).add(1))
			},
        ...
        13: {
			...
			effect:function(x){{
				let eff = n(x).mul(0.01).min(0.5).add(1)
                let eff2 = n(LiMineMain()[0]).mul(n(x).mul(0.3)).max(1).logBase(n(10).sub(n(x).mul(0.1)).max(1.1)).pow(n(x).sub(100).max(0).mul(0.01).add(1)).max(1)
				return [eff,eff2]
			}},
            style(){
                if(this.canAfford())
        ...
        21: {
			cost(x) { 
				return n(10000).pow(n(x).mul(0.2).add(1))
			},
			title(){
                let a = '未解锁'
			...
			effect:function(x){{
				let cold = n(20).sub(buyableEffect('li',22))
                let eff = n(1e4).mul(x)
				return [cold,eff]
			}},
            style(){
            ...
            shown(){return getBuyableAmount(this.layer, 11).gte(5)},
			unlocked(){return true},
		},
        22: {
			cost(x) { 
				return n(12000).pow(n(x).mul(0.12).add(1))
			},
			title(){
                let a = '未解锁'
			...
			display(){
                ...
                if(this.shown()){a = "降低矿工冷却<br>"+"消耗:"+format(this.cost())+"矿石<br>"+"效果:<br>-"+format(buyableEffect('li',22))+'冷却(上限:-10)'}
                return a
            },
			canAfford() { return player[this.layer].ore.gte(this.cost()) && this.shown()},
			buy() {
